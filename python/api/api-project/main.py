...
c = Counter("http_requests_total", "Number of http reqauests received", ["method", "endpoint"])
...
def get_sum_n(n:int):
    total_sum = 0
    c.labels(method="GET", endpoint=f"/sum_n/{n}").inc()
    h.labels(method="GET", endpoint=f"/sumn/{n}").observe(0.2)
...
def get_fibo(n:int):
    ...
    c.labels(method="GET", endpoint=f"/fibo/{n}").inc()
    h.labels(method="GET", endpoint=f"/fibo/{n}").observe(0.2)
    gfibo.set(fib2)
...
@app.get("/list")
def get_list():
    c.labels(method="GET", endpoint=f"/iist").inc()
    h.labels(method="GET", endpoint=f"/list").observe(0.2)
...
def calculator(item: Expression):
    ...
    c.labels(method="GET", endpoint=f"/calculator").inc()
    h.labels(method="GET", endpoint=f"/calculator").observe(0.2)
...
# Fixed metric names to follow best practices
c = Counter("http_requests_total", "Number of HTTP requests received", ["method", "endpoint"])
h = Histogram("http_requests_milliseconds", "Duration of HTTP requests in milliseconds", ["method", "endpoint"])
gsum = Gauge("last_sum_n", "Value stores last result of sum_n")  # Renamed from last_sum1n
...
def sanitize_endpoint(endpoint: str) -> str:
    # Replace any invalid characters in the endpoint with underscores or remove them
